 ...
 <section class="min-h-screen hero pt-20 px-6 relative overflow-hidden">
     ...
     <div class="hero-content max-w-6xl mx-auto grid md:grid-cols-2 gap-12 items-center relative z-10 w-full p-0">
         <div class="space-y-8 text-left">
             ...
             @else
                 <div
                     class="badge badge-lg badge-outline gap-2 border-primary/20 text-primary bg-primary/10 p-4 text-red-500">
                     <span class="relative flex h-2 w-2">
                         <span
                             class="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-500 opacity-75"></span>
                         <span class="relative inline-flex rounded-full h-2 w-2 bg-red-500"></span>
                     </span>
                     {{ __('Working on something else') }}
                 </div>
             @endif
             <h1 class="text-5xl md:text-7xl font-heading font-bold leading-tight">
                 {{ $shortcode->title_left }} <span class="text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent">{{ $shortcode->title_highlight }}</span> {{ $shortcode->title_right }}
             </h1>
             ...
             <div class="flex flex-wrap gap-4">
                 <a href="#projects" class="btn btn-primary gap-2">
                     {{ __('View Projects') }} {!! BaseHelper::renderIcon('ti ti-arrow-right', null, ['class' => 'w-4 h-4']); !!}
                 </a>
                 <a href="{{ $shortcode->github_url }}" target="_blank"
                     class="btn btn-neutral gap-2 border border-white/10 hover:border-white/30">
                     {!! BaseHelper::renderIcon('ti ti-brand-github', null, ['class' => 'w-4 h-4']); !!} {{ __('GitHub') }}
                 </a>
             </div>
         </div>
         <!-- Hero Visual -->
...
<code><span class="text-purple-400">class</span> <span class="text-yellow-300">Developer</span> {
  <span class="text-purple-400">constructor</span>() {
    <span class="text-red-400">this</span>.name = <span class="text-green-400">{{ $shortcode->name }}</span>;
    <span class="text-red-400">this</span>.role = <span class="text-green-400">{{ $shortcode->role }}</span>;
    <span class="text-red-400">this</span>.skills = [@foreach ($skills as $skill)
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <span class="text-green-400">"{{ $skill }}"</span>@if (!$loop->last),@endif
@endforeach
&nbsp;&nbsp;&nbsp;&nbsp;
    ];
  }
  ...
  <span class="text-blue-400">code</span>() {
    <span class="text-purple-400">return</span> <span class="text-green-400">"{{ $shortcode->return_text }}"</span>;
  }
}</code>
                        </pre>
                 </div>
